...
* Finally, run the following scripts one by one:
    ``` bash
    python src/s1_get_skeletons_from_training_imgs.py  (detect keypoints from image and output as array [action index,clip index,image index,action label,image path,18 key points coordinate] to single txt feature file one by one)
